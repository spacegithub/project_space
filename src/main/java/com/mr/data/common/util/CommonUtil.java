...
public class CommonUtil {
  ...
  public static String NOMAL_DATETIME_FORMAT = "yyyy-MM-dd HH:mm:ss SSS".intern();
  public static SimpleDateFormat sdFormat = new SimpleDateFormat(NOMAL_DATETIME_FORMAT);
  ...
  public static <T> boolean ifIsEmpty(T input, String sObjectType) {
    ...
    if (StringUtils.isNotBlank(sObjectType)) {
      if ("JsonNode".equals(sObjectType)) {
        ...
        return "{}".equals(sInput) || "".equals(sInput) || "null".equals(sInput) || "\"\"".equals(sInput)
            || "[]".equals(sInput);
      }
    }
   ...
   */
  public static boolean ifIsValidBankNo(String cardNo) {
    try {
  ...
  public static String getCurMonthFirstDay() {
    ...
    String first = sdFormat.format(c.getTime());
    // System.out.println("===============Curfirst:" + first);
    logger.info("===============Curfirst:" + first);
    return first;
  }
  ...
  public static String getCurMonthLastDay() {
    ...
    // System.out.println("===============Curlast:" + last);
    logger.info("===============Curlast:" + last);
    return last;
  }
   ...
   * @return
   */
  public static String getNextMonthFirstDay(int addNums) {
    Calendar c = Calendar.getInstance();
    c.add(Calendar.MONTH, addNums);
    c.set(Calendar.DAY_OF_MONTH, 1);// 设置为1号,当前日期既为本月第一天
    String first = sdFormat.format(c.getTime());
    // System.out.println("===============NextAddfirst:" + first);
    logger.info("===============NextAddfirst:" + first);
    return first;
  }
   ...
   * @return
   */
  public static String getNextMonthLastDay(int addNums) {
    Calendar ca = Calendar.getInstance();
    ca.add(Calendar.MONTH, addNums);
    ca.set(Calendar.DAY_OF_MONTH, ca.getActualMaximum(Calendar.DAY_OF_MONTH));
    String last = sdFormat.format(ca.getTime());
    // System.out.println("===============NextAddlast:" + last);
    logger.info("===============NextAddlast:" + last);
    return last;
  }
  ...
  public static String getOtherMonthFirstDay(String year, String month) {
    ...
    c.set(Calendar.DAY_OF_MONTH, 1);// 设置为1号,当前日期既为本月第一天
    String first = sdFormat.format(c.getTime());
    // System.out.println("===============Otherfirst:" + first);
    logger.info("===============Otherfirst:" + first);
    return first;
  }
  ...
  public static String getOtherMonthLastDay(String year, String month) {
    ...
    String last = sdFormat.format(c.getTime());
    // System.out.println("===============Otherlast:" + last);
    logger.info("===============Otherlast:" + last);
    return last;
  }
  ...
  public static String getContractNo() {
    return "C" + CommonUtil.getTodayNow().replace("/", "").replace("-", "").replace(":", "").replace(" ", "");
  }
  ...
  public static String changeFieldToObject(String sField, String sValue) {
    if (!CommonUtil.isNull(sField) && !"{}".equals(sField)) {
      if ((sField.contains(":") && sField.contains("-") && getStrNumbers(sField, "-") % 2 == 0
          && getStrNumbers(sField, ":") % 2 == 0) || !sField.contains(":")) {
        sField = sField + ",";
   ...
   * @throws Exception
   */
  public static void incDoubleValue(HashMap<String, HashMap<String, Double>> mapObject, String sID, String sField,
      double dValue) throws Exception {
    ...
    for (String s : aField) {
      ...
      map.put(s, d);
      // System.out.println(sID+" s="+s+" v="+d);
    }
  }
   ...
   * @throws Exception
   */
  public static void incIntValue(HashMap<String, HashMap<String, Integer>> mapObject, String sID, String sField,
      double dValue) throws Exception {
  ...
  public static void addStrValue(HashMap<String, HashMap<String, String>> mapUser, String sID, String sField,
  ...
  public static String doWithDateAdd(String doDate, int addDays) throws Exception {
    ...
    // System.out.println("date="+sdf.format(reDate));
    return sdf.format(reDate);
  }

  /**
   * 判断是否是有效的日期格式，是的话将其转换为日期返回
   * 
   * @param stime
   * @return
   * @throws Exception
   */
  public static boolean isVaildDate(String stime) throws Exception {
    boolean flag = true;
    SimpleDateFormat sdFormat = new SimpleDateFormat("yyyy-MM-dd");
    try {
      sdFormat.setLenient(false);
      Date date = sdFormat.parse(stime);
   ...
   * 'i':{</br>
   * &ensp;'a':{</br>
   * &emsp;'a': '初始化类型，0：默认值进行初始化，1：采用特定方法进行初始化',</br>
   * &emsp; 'b': '特殊初始化的配置定义 1：getNextSerialNo，2：getToday，3：getNowTime',</br>
   * &emsp;'c': '初始化数据的前缀',</br>
   * &emsp; 'v': '默认值进行初始化的值'</br>
   * &ensp;}, <br>
   * &ensp;'e':''<br>
   * }
   * </ul>
   ...
   * @return {boolean}
   */
  public static boolean ifIsSuccessResult(String sResult) {
    if (CommonUtil.isNull(sResult))
      return false;
    try {
      JsonNode jResult = JsonUtil.getJson(sResult);
      return ifIsSuccessResult(jResult);
    } catch (Exception e) {
      e.printStackTrace();
      return false;
    }
  ...
  public static boolean ifIsSuccessResult(JsonNode jResult) throws Exception {
    if (CommonUtil.isNullJson(jResult)) {
      return false;
    }
    String sError = JsonUtil.getJsonStringValue(jResult, "s");
    if ("0".equals(sError)) {
      return false;
    } else {
      return true;
   ...
   * @throws Exception
   */
  public static boolean ifisContainsKey(String sInput, JsonNode jInput) throws Exception {
    return ifisContainsKey(sInput, ",", jInput);
  }
   ...
   * @throws Exception
   */
  public static boolean ifisContainsKey(String sInput, String sSeparator, JsonNode jInput) throws Exception {
   ...
   * @return {String}
   */
  public static String getUnion(String sInput1, String sSeparator1, String sInput2, String sSeparator2) {
    String[] aInputs = StringUtils.split(sInput1, sSeparator1);
    for (String value : aInputs) {
      if (!ifIsIndexOf(sInput2, value, sSeparator2)) {
        sInput2 += sSeparator2 + value;
      }
    }
    return sInput2;
  }
  ...
  public static String getBackupTableName(String sTable) {
    return sTable + "_backup";
  }

  public static String getArchiveTableName(String sTable) {
    return sTable + "_archive";
  }
  ...
  public static String getCurrentTime() {
    Calendar cal = Calendar.getInstance();
    return sdFormat.format(cal.getTime()).replace("-", "").replace(":", "").replace(" ", "");
  }
  /**
   * 依照yyyy-MM的时间格式，计算两个时间相差的月份数
   * @param sFirstDate
   * @param sSecondDate
   * @return
   * @throws ParseException
   */
  public static int getSubMonth(String sFirstDate, String sSecondDate) throws ParseException {
  		SimpleDateFormat sdf  = new SimpleDateFormat("yyyy-MM");
    Calendar c = Calendar.getInstance();
    c.setTime(sdf.parse(sFirstDate));
    int year1 = c.get(Calendar.YEAR);
    int month1 = c.get(Calendar.MONTH);
     
    c.setTime(sdf.parse(sSecondDate));
  ...
  public static String[] getPathArray(String sPath) {
    String[] a = { "", "" };
  ...
  public static String getObjectClassMethodPath() {
    ...
    String sClassName = getPathArray(ste.getClassName())[1];
  ...
  private static String cityCode[] = { "11", "12", "13", "14", "15", "21", "22", "23", "31", "32", "33", "34", "35",
      "36", "37", "41", "42", "43", "44", "45", "46", "50", "51", "52", "53", "54", "61", "62", "63", "64", "65", "71",
      "81", "82", "91" };
  ...
  public static double convertPenney2YUAN(int iPenney) {
    return new Double(iPenney / 100);
  }

  /**
   * 随机数生成
   */
  public static int getRandomMath(int percent){
    Random random = new Random();
    return random.nextInt(percent);
  }
